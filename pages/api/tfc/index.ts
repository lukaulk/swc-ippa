    ...
    if (req.method === 'POST') {
        ...
        try {
            if (typeof usuario_id === 'string') {
                ...
                const allTFCs = await prisma.tfc.findMany();
                res.status(200).json({ success: true, data: allTFCs });
            }
        } catch (error) {
            console.error("Erro ao buscar os dados do TFC:", error);
            res.status(500).json({ success: false, error: "Erro ao buscar os dados do TFC" });
        }
    } else {
